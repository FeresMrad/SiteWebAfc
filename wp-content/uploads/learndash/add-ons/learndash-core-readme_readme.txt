...
* Fix - Fix learndash_reports_get_activity function.
...
* Added - Data Sharing ( Telemetry ) tab to the advanced settings.
* Added - Log viewing tab to the advanced settings.
* Added - New payments and transactions API.
...
* Fix - Allow child class of LearnDash_Gutenberg_Block to set $block_dir property so it can register Gutenberg block using block.json.

= 4.4.1.2 =

* Added filter to quiz results output
* Fixed passed shared quiz progress between different courses

= 4.4.1.1 =

* Fixed design wizard for Kadence starter templates installer

= 4.4.1 =

* Added LearnDash section to Site Health report
* Updated Setup Wizard to create profile page
* Fixed export only exporting 5 quizzes
* Fixed expand button on LearnDash User Groups output

= 4.4.0.1 =

* Added Setup Wizard
* Added in app support documentation

= 4.4.0 =

* Added Reset Password page to Setup Wizard
* Added index.php to LearnDash cache uploads directory on server to prevent file browsing
* Added Reset Password block/shortcode
* Added login form functionality to LearnDash Registration block
* Added filter in Purchase Invoice PDF to control displayed fields
* Updated currency code selector inputs in Setup Wizard and Payments settings page
* Updated failed import messaging when using LimitRequestBody setting in .htaccess
* Updated [ld_course_resume] shortcode to redirect to correct step in certain conditions
* Updated Order Overview page to display a message if the user already has access
* Fixed ensuring that media files exist when importing an export
* Fixed translation languages not being installable/downloadable when LD Licensing & Management plugin is active
* Fixed correct quiz score calculation when using single and multiple choice questions
* Fixed price formatting issues when using coupons on initial coupon activation
* Fixed issue with the LearnDash Quiz Complete block output while specifying a user ID
* Fixed password strength message on registration form
* Fixed Fill In The Blanks quiz questions when using < or > as answers
* Fixed quiz saving interval value not saving
* Fixed [visitor] and [student] shortcodes outputting content without a course ID set
* Fixed Enable Nested URLs option not enabling when enabling Shared Course Steps
* Fixed Course Selector dropdown display on Topics listing page
* Fixed issue with video progression logic on lessons and topics
* Fixed [ld_certificate] shortcode display when quiz is taken multiple times
* Fixed setting course to Buy Now/Subscribe using REST API

= 4.3.1.3 =

* Updated LMS License page messaging for admin users that don't have access to view licensing details
* Fixed issue with licensing system upgrade notice message displaying when license plugin was inactive

= 4.3.1.2 =

* Fixed issue where empty user access list prevented access to licensing verification and addons

= 4.3.1.1 =

* Updated License Visibility feature to LearnDash LMS > Settings > Advanced > License Visibility
* Fixed showing update notice when using latest version of plugin

= 4.3.1 =

* Updated removing top level LearnDash Hub menu item
* Updated LearnDash Hub menu to LearnDash LMS -> Settings -> LMS License
* Fixed issue with imported export not properly associating user progress to courses
...
* Fixed [student] shortcode not accepting user ID parameter
* Fixed protecting file downloads in LearnDash upload directory
* Fixed importing exported files error on Windows 10
...
* Fixed Expand All/Collapse functionality in Group pages
...
* Fixed auto_increment false positive warning showing in Support tab

= 4.2.1 =

* Fixed correct content encoding on report download
* Fixed video shown setting not saving in REST API
* Fixed single comma in course/group price field not being interpreted correctly
* Fixed quiz associated topic being accessible when parent lesson uses video progression
* Fixed course progression when using a sample lesson
* Fixed course resume block not determining correct post type
* Fixed search not displaying in [ld_profile] shortcode
* Fixed incorrect step redirect when using video progression on a lesson
* Fixed sending purchase success email for RazorPay payment processor
* Fixed sending purchase invoice emails for PayPal and Stripe addon
* Fixed creating course when setting materials in REST API
* Fixed lesson not associating to course when using Course Wizard
* Fixed slow query performance in REST API
* Fixed quiz summary not allowing questions to be reviewed
* Fixed learndash_quiz_completed hook not firing on a failed quiz
* Fixed course steps rearranging when shared steps is toggled on/off
